...
function ChatBotPage() {
  ...
  const fetchCameras = async () => {
    try {
      const response = await axios.get("http://192.168.144.53:5010/getCameras");
  ...
  const handleCameraSelect = async (event) => {
    ...
    if (selectedPlace) {
      try {
        ...
        await axios.post("http://192.168.144.53:5010/updateVectorStore", { place: selectedPlace });
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post("http://192.168.144.53:5010/createFlorenceDocument", formData);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await axios.post("http://192.168.144.53:5010/getResponse", {
        query: userMessage,
      });
  ...
  return (
    <div className="flex flex-col lg:flex-row items-center justify-center min-h-screen bg-gray-900 p-6 space-y-6 lg:space-y-0 lg:space-x-8">
      <div className="flex flex-col items-center flex-1">
        <img src="unisys_logo.png" alt="ZenSafe Logo" className="w-24 h-auto object-contain mb-2" />
